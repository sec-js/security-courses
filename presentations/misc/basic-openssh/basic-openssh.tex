...
My intention is to be your sparring partner today, list the parts I think must be considered.
...
Who initiates a connection, what is a port, can I use a different port, how to check if the firewall is open
...
\item We used to use Telnet, rcp, FTP and other insecure protocols a lot
\end{list1}
...
\begin{list1}
\item Cryptography is the study of how data can be encrypted
\item Cryptography uses algorithms that, together with keys, produce a
  ciphertext
\item  - which can only be read using the corresponding key
\end{list1}

\slide{Cryptographic principles}

\begin{list2}
\item The algorithms are public, the keys are secret \url{https://en.wikipedia.org/wiki/Kerckhoffs%27s_principle}
\item The algorithms are public
\item Keys have a limited lifespan -- they must be rotated frequently
\item A successful attack on a cryptographic algorithm is any shortcut
  that requires less work than exhausting all possible keys
\item New algorithms, programs, protocols, etc. must be reviewed carefully!
\item See also Snake Oil Warning Signs:
...
\begin{list1}
\item Private-key cryptography (e.g. AES) uses the same key
  for both encryption and decryption
\item Public-key cryptography (e.g. RSA) uses two
  separate keys for encryption and decryption
\end{list1}

\slide{Public Key Cryptography - 2}
\hlkimage{12cm}{images/crypto-public-key-2.pdf}
\begin{list1}
\item Public-key cryptography (e.g. RSA) uses the private
  key to decrypt
\item Public-key cryptography can also be used to
  sign documents\\ - which are then verified using the public key
\item NB: Encryption alone does not guarantee anonymity
\end{list1}
...
\item Fingerprints of severs are saved in \verb+.ssh/known_hosts+
\item Note: this is most important when using passwords!
\end{list2}
...
%      \item {\bf user@remote-server.com} --- The SSH gateway host
\item Remote Port Forwarding works the opposite, back into the client but otherwise very similar
\end{list2}
...
\slide{OpenSSH X11 Forwarding}

X11 forwarding allows running graphical applications on a remote host

\verb+ssh -Y user@host+

\begin{list1}
\item Use \verb+ssh -Y+ for trusted forwarding -- less secure, fewer restrictions
\item On OpenBSD, \verb+X11Forwarding+ is disabled by default -- enable explicitly
\item Requires \verb+xauth+ installed on the server: \verb+pkg_add xauth+
\item See also: \verb+sshd_config(5)+, \verb+ssh_config(5)+\\
...
\slide{Jump Server -- Secure SSH Access, sidenote }

\hlkimage{4cm}{skull-and-bones-aj.png}

\begin{list2}
\item A jump server is the single authorised entry point for SSH into and out of a secure network
\item All traffic is funnelled through one host -- making it auditable and monitorable
\item The jump server enforces: authentication, logging, and access control in one place
\item Without a jump server, users may establish direct SSH tunnels that bypass firewall rules and security policies
\item SSH agent forwarding and port forwarding can silently circumvent network segmentation
\item Use \verb+ProxyJump+ to connect transparently via the jump server:
\end{list2}
...
\begin{list2}
  ...
  \item Basic syntax with SSH \\
    ...
    \item Custom SSH options \\
      \verb+rsync -e "ssh -p 2222 -i ~/.ssh/id\_ed25519" ...+
...
\item Do not use the built-in VPN for sending packets over the SSH tunnel. TCP-in-TCP and all that -- use Wireguard instead
\end{list2}
...
\slide{Audit OpenSSH -- Nmap SSH Servers}
...
\item We expect updated software and on the Internet -- NO PASSWORD BASED LOGINS
\end{list2}
...
\slide{SSHD configuration}

\verb+/etc/ssh/sshd_config+:
\begin{alltt}
#Port 22
#AddressFamily any
...
Much more can be done in configuration of client in \verb+ssh_config+ and the above.

\begin{list2}
\item Special use-cases can restrict commands after login, and use special shells like \verb+git-shell+
\end{list2}

\slide{git-shell on OpenBSD}
\begin{list2}
\item \verb+git-shell+ restricts SSH users to Git operations only
\item Install Git on OpenBSD: \verb+pkg_add git+
\item Set \verb+git-shell+ as the login shell for the git user:
\begin{verbatim}
useradd -s /usr/local/bin/git-shell git
\end{verbatim}
\item Authorised keys go in \verb+~git/.ssh/authorized_keys+
\item Users can clone, push and pull -- but get no interactive shell
\item On OpenBSD, combine with \verb+doas+ rules and file permissions
  for additional containment
\item See also: \verb+git-daemon(1)+, \verb+sshd_config(5)+\\
...
There have been software issues with OpenSSH -- and other SSH implementations! so still use defense-in-depth -- like git-shell and restrict environments as much as possible. Less permissions, less damage.


\slide{Connecting to old devices and servers}

Sometimes you will find an old server, and if you are lucky you can still connect --using insecure settings \emph{temporarily}:
\begin{alltt}\scriptsize
Host 10.0.42.40
...
Only recommended until you have the data you need, and can shutdown this!
